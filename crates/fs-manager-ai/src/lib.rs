...
mod tests {
    ...
    #[test]
    fn llm_model_hf_ids_are_stable() {
        assert_eq!(LlmModel::Qwen3_4B.hf_id(), "Qwen/Qwen3-4B");
        assert_eq!(LlmModel::Qwen3_8B.hf_id(), "Qwen/Qwen3-8B");
        assert_eq!(LlmModel::Custom("x/y".into()).hf_id(), "x/y");
    }

    #[test]
    fn llm_model_from_hf_id_roundtrip() {
        assert_eq!(LlmModel::from_hf_id("Qwen/Qwen3-4B"), LlmModel::Qwen3_4B);
        assert_eq!(
            LlmModel::from_hf_id("unknown/model"),
            LlmModel::Custom("unknown/model".into())
        );
    }

    #[test]
    fn engine_status_is_running() {
        assert!(EngineStatus::Running { port: 1234 }.is_running());
        assert!(!EngineStatus::Stopped.is_running());
        assert!(!EngineStatus::Error("e".into()).is_running());
    }

    #[test]
    fn llm_engine_id_and_name() {
        let engine = LlmEngine::new(LlmConfig::default(), "/bin/fake", "/tmp/data");
        assert_eq!(engine.id(), "mistral");
        assert_eq!(engine.name(), "Mistral.rs");
    }
}
